...
```
comment: 
Before 2014, every year had 194 rows (all countries represented), with a missing rate around 40–85%. This suggests partial but wide coverage — all countries were surveyed, though some did not report all variables.
After 2014, the number of rows dropped sharply (111 → 80 → 64), and the missing rate for `TOTAL Actual DD` became almost zero. This means only a subset of countries continued to report.
